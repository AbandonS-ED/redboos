...
- 所有图片风格必须保持绝对统一"""
    ...
    STEP_CONTENT = {
        "AI资讯": {
            ...
            "步骤五": "[使用指南图，极浅深蓝背景，参数区+在单张图内展示的编号步骤列表（01-04步），每步包含编号、标题、命令/说明文字]",
        },
        "AI工具推荐": {
...
- 透明度30%，（参考透明度：30%）
- 阴影效果（参考阴影：0 4px 16px rgba(0,0,0,0.08)）

示例(错误格式，不要这样写):
❌ 嵌套括号：（参考边距：（参考像素：12px）））— 应为：（参考边距：12px）
❌ 嵌套括号：（参考阴影：0（参考像素：4px）（参考像素：16px）rgba(0,0,0,0.08)）— 应为：（参考阴影：0 4px 16px rgba(0,0,0,0.08)）
❌ 颜色直接写数值：深靛蓝色 #1a1a2e — 应为：深靛蓝色（参考色值：#1a1a2e）
❌ 字号直接写数值：96pt超粗 — 应为：（参考大小：96pt）超粗
❌ 像素直接写数值：80px留白 — 应为：（参考像素：80px）留白
❌ 混在一起：（参考边距：（参考像素：（参考像素：12px））） — 应为：（参考边距：12px）
❌ 双重嵌套任何情况都禁止"""
