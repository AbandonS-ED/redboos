...
5. 颜色代码(如#f0f4f8)、字体大小(如96pt)、像素值(如120px)等设计参数不要用双引号，它们是指令不是要显示的内容
6. 8张图片必须严格按顺序：步骤一、步骤二、步骤三、步骤四、步骤五、步骤六、步骤七、步骤八，不可缺少任何一步，不可添加额外步骤

【颜色代码位置规则】
- 颜色代码必须跟在颜色名称后面，用逗号分隔
- 正确：纯色背景，极浅深蓝色 #f0f4f8
- 错误：纯色背景 #f0f4f8（极浅深蓝色）
- 正确：深蓝色 #1a1a2e，96pt超粗
- 错误：96pt超粗深蓝色#1a1a2e

示例(正确格式):
- 底部放置文字"从安装到精通"，20pt深蓝色，距底部120px
- 中央显示标题"Claude Code"，96pt超粗，深蓝色 #1a1a2e
- 纯色背景，极浅深蓝色 #f0f4f8
- 圆角矩形背景，深蓝色 #1a1a2e

示例(错误格式):
- 底部放置"从安装到精通"，使用20pt深蓝色字体，文字与底部边缘保持120像素距离
- 标题"Claude Code"，使用96pt超粗深蓝色(#1a1a2e)字体
- 纯色背景 #f0f4f8（极浅深蓝色），无任何装饰元素"""
...
def build_prompt(content_type, topic, note_index=1, material=None):
    ...
    Args:
        ...
        material: Optional source material content
...
def build_body_prompt(topic, content_type, image_prompts, material=None):
    """Build a prompt for generating Xiaohongshu body text.

    Args:
        topic: The topic of the note
        content_type: One of "AI资讯", "AI工具推荐", "开源项目解读"
        image_prompts: List of 8 image prompt strings
        material: Optional source material content
